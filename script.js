...
const ToDoForm = () => {
    ...
    const changeToNotDoneState = (event) => {
        ...
        setToDoItems(
            toDoItems.map((item, idx) =>
                idx == index ? { item: item.item, state: 1 } : item
            )
        );
    };
    ...
    const changeToDoneState = (event) => {
        ...
        setToDoItems(
            toDoItems.map((item, idx) =>
                idx == index ? { item: item.item, state: 0 } : item
            )
        );
    };
    const removeCompleted = () => {
        setToDoItems(toDoItems.filter((item, idx) => item.state < 1));
    };
                ...
                if (item.state === 0) {
                    return (
                        <div className="text-center m-2 toDoItem" key={idx}>
                            <h1
                                style={{ color: "rgb(112, 255, 112)" }}
                                data-key={idx}
                                onClick={changeToNotDoneState}
                            >
                                {item.item}
                            </h1>
                        </div>
        ...
        <div className="row">
            <div className="col-md-8 offset-md-2 col-12  text-center">
                ...
                <h5 className="text-center text-bold">
                    Click on the text to mark item.
                </h5>
            </div>
        </div>
    );
